...
public class PlayerController : MonoBehaviour {
    ...
    {        
        if (col.gameObject.tag == "Enemy")
        {
            ...
            {
                playerBody.AddForce(Vector2.left * 10, ForceMode2D.Impulse);
            }
            else
            {
                playerBody.AddForce(Vector2.right * 10, ForceMode2D.Impulse);
            }
            playerHurt.Play();
        }
    }
    ...
    void OnTriggerEnter2D(Collider2D col)
    {
        ...
        if (col.gameObject.tag == "Enemy")
        {
            ...
            col.gameObject.GetComponent<enemyDeath>().enemyDeathInstantinate(col.gameObject.transform.position);
        ...
        if (col.gameObject.tag == "Chicken")
        {
            ...
            col.gameObject.GetComponent<itemPickup>().itemPickupInstantinate(col.gameObject.transform.position);            
            Destroy(col.gameObject);
        }
    ...
    void checkHealth()
    {
        ...
        {
            Die();
            Debug.Log("Player dont have a health");
        }
    }
}
